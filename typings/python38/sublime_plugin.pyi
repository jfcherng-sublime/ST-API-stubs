...
def load_module(m):
    ...
    if el_on_activated_async_targets or vel_on_activated_async_targets:
        with pending_on_activated_async_lock:
            pending_on_activated_async_callbacks["EventListener"].extend(el_on_activated_async_targets)
            pending_on_activated_async_callbacks["ViewEventListener"].extend(vel_on_activated_async_targets)
...
def attach_view(view):
    ...
    view.settings().add_on_change("check_view_event_listeners", lambda: check_view_event_listeners(view))
...
def normalise_completion(c):
    ...
    if not isinstance(c, sublime.CompletionItem):
        if isinstance(c, str):
            ...
            trigger, annotation = split_trigger(c[0])
            c = sublime.CompletionItem.snippet_completion(trigger, c[1], annotation, kind=sublime.KIND_AMBIGUOUS)
        elif len(c) == 3:
            trigger, annotation = split_trigger(c[0])
            c = sublime.CompletionItem.snippet_completion(trigger, c[2], annotation, kind=sublime.KIND_AMBIGUOUS)
        else:
...
class MultiCompletionList:
    ...
    def completions_ready(self, completions, flags):
        ...
        if self.remaining_calls == 0:
            sublime_api.view_set_completions(self.view_id, self.req_id, (self.completions, self.flags))
        ...
        for item in items:
            if isinstance(item, str):
                ...
                if item.annotation != "" or item.kind != (sublime.KIND_ID_AMBIGUOUS, "", ""):
                    ...
                    item_tuples.append(
                        ((item.text, details, item.annotation, (item.kind[0], kind_letter, item.kind[2])), item.value)
                    )
            ...
            if "required positional argument" in str(e):
                if sublime_api.can_accept_input(self.name(), args):
                    sublime.active_window().run_command(
                        "show_overlay", {"overlay": "command_palette", "command": self.name(), "args": args}
                    )
                    return
            raise
...
class EventListener:
    ...
    .. method:: on_query_context(\
            view: View, key: str, operator: QueryOperator, operand: str, match_all: bool) -> bool|None
...
class ViewEventListener:
    ...
    .. method:: on_query_context(key: str, operator: QueryOperator, operand: str, match_all: bool) -> bool|None
...
class MultizipImporter(importlib.abc.MetaPathFinder):
    ...
    def _make_spec(self, loader, fullname):
        ...
        spec = importlib.util.spec_from_loader(fullname, loader, origin=origin, is_package=is_package)
        if is_package:
...
class ZipLoader(importlib.abc.InspectLoader):
    ...
    def get_code(self, fullname):
        ...
        if not info[0].endswith(".pyc"):
            return importlib.abc.InspectLoader.source_to_code(self._load_source(fullname, info[0]), info[0])
                ...
                for f in files:
                    ...
                    if f.endswith(".pyc"):
                        ...
                        try:
                            ...
                            print(f"{os.path.join(self.zippath, f)} is not utf-8 encoded, unable to load plugin")
                            continue
